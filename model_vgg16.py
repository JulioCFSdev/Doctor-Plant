...
sdir=r'dataset\Image Data base\Image Data base'
...
# Treina o modelo
trained_mod = model.fit(train_gen, validation_data=valid_gen, epochs=1, callbacks=[earlystop], workers = workers)
...
def tr_plot(trained_data, start_epoch):
    #Plotando o treinamento e validação dos dados 
    tacc=trained_data.history['accuracy']
    tloss=trained_data.history['loss']
    vacc=trained_data.history['val_accuracy']
    vloss=trained_data.history['val_loss']
    Epoch_count=len(tacc)+ start_epoch
    Epochs=[]
    for i in range (start_epoch ,Epoch_count):
        Epochs.append(i+1)
    # Epoca com a menos validação de perda
    ...
    axes[0].set_title('Curva de perda do treinamento e validação')
    ...
    axes[1].set_title('Acurácia do treinamento e validação')
